...
class List {//Ataken from the 2Task
public:
	List() { head = NULL; };
	~List();
	struct node {
...
public:
	...
	void addhint(char *a, char *h); //aadding hint "h" and answer "a"
...
void Game::play(){//new try START
	...
	if(!strcmp(answer, code))//aif u are VERY LUCKY  and won output win
		win();
	else{
		...
		Hints.addhint(answer, hint);//aadd the hint generated to file
...
void Game::javaProv(){//iiiyoii ooo ana
...
//updated the iner vycle is now there
...
parser::parser() {
	...
	char* ptr = key; //variable to write to
	char * str = query();
