...
void Game::play(){//first try START
	char * code = gen();//agenerate the cor answer
	
	if(!strcmp(answer, code))//aif u are VERY LUCKY  and won from the 1st try, output win
		win();
	else{
		lose();//if not then output lost and rewrite files and the html
		char * hint = check(answer, code);//iusing our guess generate the hint for the next try
...
void Game::javaProv(){//to check the tries sent locally
	...
	cout<< "var s = document.getElementById(\"code\");\n";	

	
	cout<< "s.addEventListener('blur', chk, true);\n";
...
char * Game::gen(){//ngenerate the correct answer for the hint
...
char * Game::check(char *a, char *c){//create string for hint
	...
	temp[4]='\0';//adding the termination elem to  end the string
	
	return temp;
}


// oa?aou a?aiaiio? ia?aiaiio?
//?oia n?aco ia?aaaaaeny eeno
//ia?aienaou iianeacee e aaia?aoey i?aaeeuiiai ioaaoa n iiiiuu? aei?aiuo oeeeia

//2 caaa?a oa?aou a?aiaiio? ia?aiaiio?
//ia?aianoe eiinoaioo 500 a eeann
//i?iaa?ea ia oieone?iaea a ia ia ioi?aaeaiee oi?iu
...
void Game::newfile(char *key, char * a, char *hint){//generating file and writing all of info threre 4 future work
	...
	info << key << endl;//i?aaeeuiue ioaao
	info << 1 << endl;//iiiuoea 1. Oae eae yoi ia?aay ea?a
	info.close();//cae?uaaai oaee aey caiene
...
//the pointer poiants to one buffer after another; this or 2 pointers will be needed
parser::parser() {
	int i = 0;
	char value[100]; //value buffer
	char key[100];  //key buffer
	char* ptr = key; //pointer to key or value string 
	char * str = query();
